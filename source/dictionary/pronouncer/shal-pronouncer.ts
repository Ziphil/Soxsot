...
export class ShalPronouncer extends Pronouncer {

  private showSyllables: boolean;
  private light: boolean;

  public constructor(configs?: ShalPronouncerConfigs) {
    super();
  ...
  public convert(name: string): string | null {
    if (name === "kin") {
      ...
      let syllableName = ShalPronouncer.divideSyllables(name);
  ...
  private static divideSyllables(name: string): string {
    ...
    dividedName = dividedName.replace(/((<[sztdkgfvpbcqxjlrnmyh]>)?<[aeiouâêîôûáéíóúàèìòù]>(<[sztdkgfvpbcqxjlrnmyh]>)?)/g, "$1.");
    const regexp = /(<.>|\.)/g;
    const array = [];
    let match;
...
export type ShalPronouncerConfigs = {
