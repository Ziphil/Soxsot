...
export * from "./single-loader";


export function createLoaderByKind<K extends LoaderKind>(kind: K, ...args: ConstructorParameters<LoaderClass<K>>): InstanceType<LoaderClass<K>> {
  const Loader = LOADER_DATA[kind].clazz as any;
  const loader = new Loader(...args);
  return loader;
}

export function createLoaderByExtension(path: string): Loader | undefined {
