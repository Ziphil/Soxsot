//

export * from "./dictionary/loader/deserializer";
export * from "./dictionary/parameter/normal-parameter";
export * from "./dictionary/parameter/parameter";
export * from "./dictionary/saver/file-name-resolver";
export * from "./dictionary/saver/serializer";
export * from "./dictionary/dictionary";
export * from "./dictionary/dictionary-settings";
export * from "./dictionary/equivalent";
export * from "./dictionary/information";
export * from "./dictionary/marker";
export * from "./dictionary/parsed-word";
export * from "./dictionary/parser";
export * from "./dictionary/part";
export * from "./dictionary/relation";
export * from "./dictionary/revision";
export * from "./dictionary/section";
export * from "./dictionary/search-result";
export * from "./dictionary/suggestion";
export * from "./dictionary/word";


export const VERSION = "0.2.0";